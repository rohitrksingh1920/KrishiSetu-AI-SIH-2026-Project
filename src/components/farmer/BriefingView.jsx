...
export default function BriefingView({ lang }) {
  ...
  const greet =
    hour < 12 ? t.greetMorning : hour < 17 ? t.greetAfternoon : t.greetEvening;
    ...
    <div className="ks-rise">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3 mb-5">
        <div>
          ...
          <h1 className="ks-display text-[26px] sm:text-[30px] font-semibold leading-tight">
            {t.briefing}
          </h1>
      ...
      <div className="grid gap-3">
        {actions.map((a, i) => {
          ...
          return (
            <div
              key={i}
              className="ks-card ks-liftable rounded-lg p-4"
              style={{ borderLeft: `4px solid ${a.tone}` }}
            >
              <div className="flex items-start gap-3">
                <div
                  className="w-9 h-9 rounded-md flex items-center justify-center shrink-0"
                  style={{ background: `${a.tone}1A` }}
                >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-3">
                    <h3
                      className={`font-semibold text-[15px] leading-snug ${isDone ? "line-through opacity-50" : ""}`}
                    >
                      {a.title[lang]}
                    </h3>
                  ...
                  {expanded === i && (
                    <p className="text-[13px] mt-1.5 opacity-80 leading-relaxed ks-rise">
                      {a.why[lang]}
                    </p>
                  )}
      ...
      <div
        className="mt-5 flex items-center gap-2 text-[12px] px-3 py-2.5 rounded-md"
        style={{ background: `${C.monsoon}0D`, color: C.monsoonDeep }}
      >
        <WifiOff size={14} />
        Last synced 6h ago while offline. This briefing is cached and will
