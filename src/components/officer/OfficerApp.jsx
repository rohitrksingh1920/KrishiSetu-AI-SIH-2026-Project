...
import { dataSources } from "../../data/pitchData.js";
import SectionEyebrow from "../primitives/SectionEyebrow.jsx";
import SourceChip from "../primitives/SourceChip.jsx";
import ValidationPipeline from "./ValidationPipeline.jsx";

const sourceIcon = {
  "IMD Weather API": Wifi,
  "Bhashini API": Wifi,
  "Copernicus Sentinel-2": Satellite,
  "AgriStack / UFSI": Building2,
  "Soil Health Card": FileText,
  PMFBY: ShieldCheck,
};
const toneMap = {
  ndvi: C.ndvi,
  ...
  return (
    <div className="flex-1 flex min-h-0">
        ...
        {officerNav.map((n) => {
          ...
          return (
            <button
              ...
              style={{
                background: active ? "rgba(217,164,4,0.18)" : "transparent",
                color: active ? C.turmeric : C.paper,
              }}
            >
      ...
      <main className="flex-1 min-w-0 overflow-y-auto ks-scroll p-5 sm:p-8">
        {tab === "overview" && (
          <div className="ks-rise">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-3">
              {[
                ...
                <div key={l} className="ks-card rounded-lg p-4">
                  <div
                    className="ks-display text-[26px] font-semibold"
            ...
            <p className="text-[12px] opacity-55 mt-4 max-w-md">
              All figures aggregated and anonymized — no individual farmer
              records shown outside authenticated officer sessions.
            </p>
        ...
        {tab === "farmers" && (
          <div className="ks-rise">
            <h1 className="ks-display text-[24px] font-semibold mb-4">
              Farmers &amp; plots
            </h1>
                ...
                <thead>
                  <tr style={{ background: C.paperDeep }}>
                    {[
                      ...
                      <th
                        key={h}
                        className="text-left px-3 py-2 ks-mono text-[10.5px] uppercase tracking-wide opacity-70"
                      >
                        {h}
                      </th>
                    ))}
          ...
          <div className="ks-rise">
            ...
            <div className="grid gap-2.5">
              {escalations.map((e) => (
                <div
                  key={e.id}
                  className="ks-card rounded-lg p-3.5 flex items-center gap-3"
                >
                  <AlertTriangle
                    size={16}
                    style={{ color: e.sla === "Closed" ? C.ndvi : C.alert }}
                  />
                  <div className="flex-1 min-w-0">
                    <div className="text-[13px] font-medium">
                      {e.farmer}{" "}
                      <span className="opacity-50 ks-mono text-[11px]">
                        · {e.id}
                      </span>
                    </div>
                  ...
                  <div
                    className="ks-mono text-[11.5px] w-16 text-right"
                    style={{ color: e.sla === "Closed" ? C.ndvi : C.alert }}
                  >
                    {e.sla}
                  </div>
            ...
            <div className="grid sm:grid-cols-3 gap-3">
              {claims.map((c) => (
                <div key={c.id} className="ks-card rounded-lg p-4">
                  ...
                  <div className="font-semibold text-[14px] mt-0.5">
                    {c.farmer}
                  </div>
                  <div className="text-[12.5px] opacity-70 mt-0.5">
                    {c.cause}
                  </div>
        ...
        {tab === "system" && (
          <div className="ks-rise">
            ...
            <p className="text-[12.5px] opacity-65 mb-4 max-w-lg">
              Live integration status of every government data source and API
              this prototype is built on top of.
            </p>
            <div className="grid sm:grid-cols-2 gap-3 max-w-2xl">
              {dataSources.map((d) => {
                const Icon = sourceIcon[d.name] || Wifi;
