...
import { AlertTriangle, Wifi, Satellite, Building2, FileText, ShieldCheck } from "lucide-react";
...
const sourceIcon = { "IMD Weather API": Wifi, "Bhashini API": Wifi, "Copernicus Sentinel-2": Satellite, "AgriStack / UFSI": Building2, "Soil Health Card": FileText, PMFBY: ShieldCheck };
const toneMap = { ndvi: C.ndvi, turmeric: C.turmeric, soil: C.soil, neutral: "#8A8368" };
      ...
      <main className="flex-1 min-w-0 overflow-y-auto ks-scroll p-5 sm:p-8">
        ...
        {tab === "system" && (
          <div className="ks-rise">
            ...
            <p className="text-[12.5px] opacity-65 mb-4 max-w-lg">Live integration status of every government data source and API this prototype is built on top of.</p>
            <div className="grid sm:grid-cols-2 gap-3 max-w-2xl">
              {dataSources.map((d) => {
                const Icon = sourceIcon[d.name] || Wifi;
                const tone = toneMap[d.tone];
                return (
                  <div key={d.name} className="ks-card rounded-lg p-3.5 flex items-center gap-3">
